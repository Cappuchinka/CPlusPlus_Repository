...
Label::Label(int x, int y, int width, int height, bool visible, std::string name) :
    Component(x, y, width, height, visible, name) {}

std::string Label::getInfo() {
    std::string str = "Label have next params:";
...
std::string Label::getInfo(int n) {
    std::string str = "Label have next params";
    std::string tab(n, '\t');
...
void Label::setNewXY(int newX, int newY) {
    x = newX;
    y = newY;
}
